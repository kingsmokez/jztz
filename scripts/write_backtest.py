...
def backtest_current_picks(top_n=20):
    ...
    for i, stock in enumerate(stocks):
        ...
        df = get_history_kline(code, days=90)
        if df.empty:
        ...
        if len(recent) < 5:
            ...
            continue
        highs = recent['\u6700\u9ad8'].values.astype(float)
        lows = recent['\u6700\u4f4e'].values.astype(float)
    ...
    print('--- Hit Rate ---')
    print('Buy hit: {:.1f}% ({}/{})'.format(buy_rate, br, total))
    print('Sell hit: {:.1f}% ({}/{})'.format(sell_rate, sr, total))
    print('Stop hit: {:.1f}% ({}/{})'.format(stop_rate, strr, total))
    print('--- Trade Path ---')
    print('Buy->Sell: {:.1f}% ({}/{})'.format(buy_sell_rate, bts, br))
    print('Buy->Stop: {:.1f}% ({}/{})'.format(buy_stop_rate, btst, br))
    print('--- Magnitude ---')
    print('Pred upside: {:.1f}%'.format(avg_upside_pred))
    print('Actual max gain: {:.1f}%'.format(avg_upside_actual))
    ...
    print('Max gain avg: {:.1f}%'.format(avg_max_gain))
    print('Max loss avg: {:.1f}%'.format(avg_max_loss))
    print('--- Risk/Reward ---')
    print('Pred RR: {:.2f}'.format(avg_rr_pred))
    print('Actual RR: {:.2f}'.format(avg_rr_actual))

    print('\n--- Diagnosis ---')
    if buy_rate > 80:
        print('!! Buy discount too small! >80% hit rate')
    elif buy_rate < 30:
        print('!! Buy discount too large! <30% hit rate')
    else:
        print('OK Buy discount reasonable')
    if sell_rate < 20:
        print('!! Sell target too high! <20% hit rate')
    elif sell_rate > 60:
        print('!! Sell target too low! >60% hit rate')
    else:
